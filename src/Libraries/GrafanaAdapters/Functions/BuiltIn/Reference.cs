...
/// in seconds, for the time slice. Parameter <c>adjustCoordinateMidPoints</c>, optional, is a boolean flag that determines if the
...
/// </summary>
/// <remarks>
/// Signature: <c>Reference(sliceTolerance, [adjustCoordinateMidPoints = false], [applyWrapOps = true], [units = Degrees], expression)</c><br/>
...
public abstract class Reference<T> : GrafanaFunctionBase<T> where T : struct, IDataSourceValueType<T>
{
    ...
    public override ParameterDefinitions ParameterDefinitions => new List<IParameter>
    {
        ...
        {
            Name = "adjustCoordinateMidPoints",
            Default = false,
    ...
    {
        bool adjustCoordinateMidPoints = parameters.Value<bool>(0);
        ...
        if (adjustCoordinateMidPoints)
        {
            // Store reference coordinates for later adjustment
            if (adjustCoordinateMidPoints = metadataMaps.TryGetValue(enumerator.Current.Target, out refCoordinates) && refCoordinates is not null)
            {
                // Only need to adjust mid-point once for the series metadata, if key exists, mid-point has already been applied
                adjustCoordinateMidPoints = !refCoordinates.ContainsKey("MidPointApplied") && tryParseCoordinates(refCoordinates, out refLongitude, out refLatitude);
            }
        }
