...
public class MovingValueInputAdapter : InputAdapterBase
{
    ...
    {
        get => m_randomNumberSeed;
        set => m_randomNumberSeed = value;
    }
    ...
    public override void Initialize()
    {
        ...
        if (!settings.TryGetValue("minValue", out string? setting) || !double.TryParse(setting, out m_minValue))
        ...
        ValueWraps = settings.TryGetValue("valueWraps", out setting) && setting.ParseBoolean();
        ...
        IncludeTime = !settings.TryGetValue("includeTime", out setting) || setting.ParseBoolean();
    ...
    {
        return $"{ProcessedMeasurements:N0} values generated between {m_minValue} and {m_maxValue} so far...".CenterText(maxLength);
    }
    ...
    private void Timer_Elapsed(object? sender, ElapsedEventArgs elapsedEventArgs)
    {
        ...
        {
            double delta = Ticks.ToSeconds(nextPublication - m_lastPublication);

            for (int i = 0; i < OutputMeasurements!.Length; i++)
                Advance(i, delta);

            OnNewMeasurements(OutputMeasurements
                .Select((measurement, index) => Measurement.Clone(measurement, GetWrappedValue(index), IncludeTime ? nextPublication : 0L))
                .ToList<IMeasurement>());
    ...
    private void Advance(int index, double time)
    {
        ...
        {
            remainingTime = m_moveFlags[index] ? 
                Move(index, remainingTime) : 
                Hold(index, remainingTime);
    ...
    {
        if (ValueWraps && m_values[index] != m_maxValue)
