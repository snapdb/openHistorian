...
using ConfigurationCell = Gemstone.PhasorProtocols.Anonymous.ConfigurationCell;
...
public class PhasorMeasurementMapper : InputAdapterBase
{
    ...
    public override string Status
    {
        get
        {
            ...
            status.AppendLine($"No data reconnect interval: {Ticks.FromMilliseconds(m_dataStreamMonitor?.Interval ?? 0).ToSeconds():0.000} seconds");
    ...
    private void LoadDeviceMeasurements()
    {
        ...
        foreach (DataRow row in DataSourceLookups.GetLookupCache(DataSource).ActiveMeasurements.LookupByDeviceID(SharedMappingID))
        {
            ...
            try
            {
                ...
                definedMeasurements.TryAdd(signalReference, key);
            }
            catch (Exception ex)
            {
    ...
    public override string GetShortStatus(int maxLength)
    {
        ...
        if (m_frameParser is not null && m_frameParser.IsConnected)
        {
            if (LastReportTime > 0L)
            {
                ...
                long totalDataErrors = DefinedDevices?.Sum(definedDevice => definedDevice.DataQualityErrors) ?? 0L;
            ...
            {
                status.Append($"  >> Awaiting configuration frame - {m_frameParser.TotalBytesReceived} bytes received");
            }
            else
            {
                status.Append($"  ** No data parsed yet. Check mapping configuration - {m_frameParser.TotalBytesReceived} bytes received");
            }
        }
        else
        {
    ...
    protected void ExtractFrameMeasurements(IDataFrame frame)
    {
        ...
        List<IMeasurement> deviceMappedMeasurements = [];
        ...
        foreach (IDataCell? parsedDevice in frame.Cells)
        {
            try
            {
                ...
                if (m_labelDefinedDevices is not null &&
                    ...
                    (m_definedDevices?.TryGetValue(parsedDevice.IDCode, out statisticsHelper) ?? false))
                {
                    ConfigurationCell definedDevice = statisticsHelper.Device;
    ...
    private int CountMappedMeasurementsWithError(IDataCell parsedDevice, List<IMeasurement> mappedMeasurements)
    {
        ...
        bool hasError(MeasurementStateFlags stateFlags)
        {
    ...
    public string GetSignalReference(SignalKind type)
    {
        ...
        references = [SignalReference.ToString($"{Name}!IS", type)];
    ...
    private void StartMeasurementCounter()
    {
        ...
        if (dataSource is not null && dataSource.Tables.Contains("ActiveMeasurements"))
        {
            ...
            foreach (DeviceStatisticsHelper<ConfigurationCell> statisticsHelper in statisticsHelpers ?? [])
            {
                ...
                statisticsHelper.ExpectedMeasurementsPerSecond = (configurationFrame?.FrameRate ?? 30) * measurementsPerFrame;
    ...
    protected virtual string GetConnectionID(IServer? server, Guid clientID)
    {
        ...
        try
        {
            ...
            {
                TcpServer commandChannel when commandChannel.TryGetClient(clientID, out TransportProvider<Socket>? tcpClient) => tcpClient?.Provider?.RemoteEndPoint as IPEndPoint,
                UdpServer dataChannel when dataChannel.TryGetClient(clientID, out TransportProvider<EndPoint>? udpClient) => udpClient?.Provider as IPEndPoint,
                _ => null
            };
    ...
    protected virtual void DeviceCommandHandler(Guid clientID, string connectionID, byte[] commandBuffer, int length)
    {
        try
        {
            ...
            {
                PhasorProtocol.IEEEC37_118V2 or PhasorProtocol.IEEEC37_118V1 or PhasorProtocol.IEEEC37_118D6 => new Gemstone.PhasorProtocols.IEEEC37_118.CommandFrame(commandBuffer, 0, length),
            ...
            switch (commandFrame.Command)
            {
                ...
                case DeviceCommand.SendConfigurationFrame3:
                    ...
                    if (m_configurationFrame is not null)
                    {
                        if (m_configurationFrame is Gemstone.PhasorProtocols.IEEEC37_118.ConfigurationFrame3 configFrame3)
                        {
                            int frames = 0;
    ...
    {
        if (m_frameParser is null || m_dataStreamMonitor is null)
            return;
    ...
    private void m_frameParser_ConnectionEstablished(object? sender, EventArgs e)
    {
        ...
        if (m_dataStreamMonitor is not null && m_frameParser is not null)
