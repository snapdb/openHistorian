...
using AnonymousConfigurationCell = Gemstone.PhasorProtocols.Anonymous.ConfigurationCell;
using AnonymousConfigurationFrame = Gemstone.PhasorProtocols.Anonymous.ConfigurationFrame;
using AnonymousDigitalDefinition = Gemstone.PhasorProtocols.Anonymous.DigitalDefinition;
using AnonymousPhasorDefinition = Gemstone.PhasorProtocols.Anonymous.PhasorDefinition;
...
public class Concentrator : PhasorDataConcentratorBase
{
    ...
    public static ConfigurationFrame3 CreateConfigurationFrame3(AnonymousConfigurationFrame baseConfigurationFrame, uint timeBase, LineFrequency nominalFrequency, Concentrator parent = null)
    {
        ...
        {
            return replaceWithSpaceChar == char.MinValue ? label : label.Replace(' ', replaceWithSpaceChar);
        }
        ...
        // CreateConfigurationFrame3 function to still be usable in a use cases outside concentrator context.
        if (!loadFromCache)
        {
            try
            {
        ...
        if (loadFromCache)
        {
            try
            {
                ...
                foreach (ConfigurationCell3 cell in configurationFrame.Cells)
                {
                    ...
                    // The following values have long been available in Gemstone time-series configuration
