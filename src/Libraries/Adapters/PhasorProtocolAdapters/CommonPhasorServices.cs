...
public sealed class CommonPhasorServices : FacileActionAdapterBase
{
    ...
    public IConfigurationFrame RequestDeviceConfiguration(string connectionString)
    {
        ...
        if (m_frameParser is null)
            throw new InvalidOperationException("Frame parser is not available for configuration request.");
    ...
    // TODO: Re-enable these features when needed
    ...
    //    try
    //    {
    //        dynamic settings = ConfigSettings.Default["ThresholdSettings"];

    //        double maxFilteredStatusMessageCacheRange = settings[MaxFilteredStatusMessageCacheRange, DefaultMaxFilteredStatusMessageCacheRange, "Maximum number of minutes allowed for any filtered status message cache"];

    //        return maxFilteredStatusMessageCacheRange;
    //    }
    //    catch (Exception ex)
    //    {
    //        Logger.SwallowException(ex, "Failed to get maximum filtered status message cache range");

    //        return DefaultMaxFilteredStatusMessageCacheRange;
    //    }
    //}
    ...
    private static TemplatedExpressionParser InitializePointTagExpressionParser()
    {
        ...
        try
        {
            ...
            string pointTagNameExpression = settings["PointTagNameExpression", DefaultPointTagNameExpression, "Defines the expression used to create point tag names. NOTE: if updating this setting, synchronize value in both the manager and service config files"];
    ...
    private static void PhasorDataSourceValidation(AdoDataConnection database, string nodeIDQueryString, ulong trackingVersion, string arguments, Action<string> statusMessage, Action<Exception> processException)
    {
        ...
        //    IEnumerable<DataRow> signalTypes = database.Connection.RetrieveData("SELECT Name, Acronym FROM SignalType WHERE LongAcronym='Undefined'").AsEnumerable();
        ...
        //IEnumerable<DataRow> statistics = database.Connection.RetrieveData("SELECT * FROM Statistic ORDER BY Source, SignalIndex").AsEnumerable();
        ...
        //HashSet<int> measurementIDsToDelete = [];
        ...
        //DataTable protocols = database.Connection.RetrieveData("SELECT * FROM Protocol");
        ...
        //        database.Connection.ExecuteNonQuery("INSERT INTO Protocol(Acronym, Name, Type, Category, AssemblyName, TypeName, LoadOrder) VALUES('STTP', 'Streaming Telemetry Transport Protocol', 'Measurement', 'Gateway', 'sttp.Gemstone.dll', 'sttp.DataSubscriber', 10)");

        //    if (Convert.ToInt32(database.Connection.ExecuteScalar("SELECT COUNT(*) FROM Protocol WHERE Acronym=\'GatewayTransport\'")) == 0)
        //        database.Connection.ExecuteNonQuery("INSERT INTO Protocol(Acronym, Name, Type, Category, AssemblyName, TypeName, LoadOrder) VALUES('GatewayTransport', 'Gateway Exchange Protocol (GEP)', 'Measurement', 'Gateway', 'Gemstone.Timeseries.dll', 'Gemstone.Timeseries.Transport.DataSubscriber', " + (++protocolCount) + ")");
        ...
        //    DataTable timeQualityProtocols = database.Connection.RetrieveData("SELECT ID FROM Protocol WHERE Acronym = 'IeeeC37_118V1' OR Acronym = 'IeeeC37_118V2' OR Acronym = 'IeeeC37_118D6' OR Acronym = 'Iec61850_90_5'");
        ...
        //    foreach (DataRow device in database.Connection.RetrieveData($"SELECT * FROM Device WHERE ((IsConcentrator = 0 AND ParentID IS NULL) OR IsConcentrator = 1) AND NodeID = {nodeIDQueryString} AND ProtocolID IN ({timeQualityProtocolIDs})").Rows)
        //    {
        ...
        //    foreach (DataRow device in database.Connection.RetrieveData($"SELECT * FROM Device WHERE IsConcentrator = 0 AND NodeID = {nodeIDQueryString} AND ProtocolID IN ({protocolIDs})").Rows)
        //    {
        ...
        //    foreach (DataRow inputStream in database.Connection.RetrieveData($"SELECT * FROM Device WHERE (IsConcentrator = 0 AND ParentID IS NOT NULL) AND NodeID = {nodeIDQueryString} AND ProtocolID IN ({protocolIDs})").Rows)
        //    {
        ...
        //    foreach (DataRow outputStream in database.Connection.RetrieveData($"SELECT * FROM OutputStream WHERE NodeID = {nodeIDQueryString}").Rows)
        //    {
        ...
        //        foreach (DataRow outputStreamMeasurement in database.Connection.RetrieveData($"SELECT * FROM OutputStreamMeasurement WHERE AdapterID = {adapterID} AND NodeID = {nodeIDQueryString}").Rows)
        //        {
        ...
        //    foreach (DataRow measurement in database.Connection.RetrieveData("SELECT SignalID, CompanyAcronym, DeviceAcronym, VendorDeviceID, SignalReference, SignalAcronym, PhasorLabel, Phase, BaseKV, AlternateTag FROM MeasurementDetail WHERE SignalAcronym <> 'STAT' AND SignalAcronym <> 'CALC' AND Internal <> 0 AND Subscribed = 0").Rows)
        //    {
    ...
    private static void EstablishDefaultMeasurementKeyCache(AdoDataConnection database, Action<string> statusMessage, Action<Exception> processException)
    {
        ...
        foreach (DataRow measurement in database.Connection.RetrieveData("SELECT ID, SignalID, SignalType, PointTag, Adder, Multiplier FROM ActiveMeasurement").Rows)
        {
