...
//       Added code to detect data set changes (i.e., PMUs online/offline).
...
public class ReferenceAngle : CalculatedMeasurementBase
{
    ...
    private readonly Dictionary<MeasurementKey, double> m_lastAngles = [];
    private readonly Dictionary<MeasurementKey, double> m_unwrapOffsets = [];
    private readonly List<double> m_latestCalculatedAngles = [];
    ...
    public override string Status
    {
        get
        {
            ...
            {
                status.Append(m_latestCalculatedAngles.Count > ValuesToShow ? 
                    m_latestCalculatedAngles.GetRange(m_latestCalculatedAngles.Count - ValuesToShow, ValuesToShow).Select(v => v.ToString("0.00°")).ToDelimitedString(", ") : 
    ...
    public override void Initialize()
    {
        ...
        List<MeasurementKey> validInputMeasurementKeys = [];

        if (InputMeasurementKeys is null || InputMeasurementKeyTypes is null)
            throw new InvalidOperationException("No input measurements were specified for the reference angle calculator.");
        ...
        if (OutputMeasurements is null || OutputMeasurements.Length < 1)
    ...
    protected override void PublishFrame(IFrame frame, int index)
    {
        ...
        if (TryGetMinimumNeededMeasurements(frame, out IMeasurement[]? measurements))
        {
            // See if data set has changed since last run
            if (m_lastAngles.Count > 0 && m_lastAngles.Count == measurements.Length)
            {
                if (measurements.Any(measurement => !m_lastAngles.ContainsKey(measurement.Key)))
                    dataSetChanged = true;
            }
            else
            {
            ...
            if (dataSetChanged)
            {
                ...
                double angleRef = measurements[0].AdjustedValue;
            ...
            angleAverage = (angleTotal / measurements.Length) % 360.0D;
        ...
        OnNewMeasurements([calculatedMeasurement]);
