...
[UIResource("AdaptersUI", $".{nameof(PowerCalculations)}.{nameof(SequenceCalculator)}.{nameof(PowerCalculations)}.{nameof(SequenceCalculator)}.js", $"{nameof(PowerCalculations)}.{nameof(SequenceCalculator)}.js")]
...
public class SequenceCalculator : CalculatedMeasurementBase
{
    ...
    // Fields
    private MeasurementKey[] m_angles = default!;
    private MeasurementKey[] m_magnitudes = default!;
    private string m_magnitudeUnits = "undefined";
    private readonly ConcurrentQueue<double> m_positiveSequenceMagnitudeSample = [];
    private readonly ConcurrentQueue<Angle> m_positiveSequenceAngleSample = [];
    private readonly ConcurrentQueue<double> m_negativeSequenceMagnitudeSample = [];
    private readonly ConcurrentQueue<Angle> m_negativeSequenceAngleSample = [];
    private readonly ConcurrentQueue<double> m_zeroSequenceMagnitudeSample = [];
    private readonly ConcurrentQueue<Angle> m_zeroSequenceAngleSample = [];
    ...
    public override void Initialize()
    {
        ...
        if (InputMeasurementKeys is null || InputMeasurementKeyTypes is null)
            throw new InvalidOperationException("No input measurements were specified for the sequence calculator.");
        ...
        if (OutputMeasurements is null || OutputMeasurements.Length == 0)
            throw new InvalidOperationException("No output measurements were specified for the sequence calculator.");
