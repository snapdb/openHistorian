...
public class ReferenceMagnitude : CalculatedMeasurementBase
{
    ...
    public override void Initialize()
    {
        ...
        if (InputMeasurementKeys is null || InputMeasurementKeyTypes is null)
            throw new InvalidOperationException("No input measurements were specified for the reference magnitude calculator.");
    ...
    protected override void PublishFrame(IFrame frame, int index)
    {
        if (frame.Measurements.Count > 0)
        {
            ...
            OnNewMeasurements([Measurement.Clone(OutputMeasurements![0], m_referenceMagnitude, frame.Timestamp)]);
        }
        else
        {
