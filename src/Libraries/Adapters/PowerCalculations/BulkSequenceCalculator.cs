...
public class BulkSequenceCalculator : IndependentActionAdapterManagerBase<SequenceCalculator>
{
    ...
    private class AdapterDetail
    {
        ...
        public string CustomSettings { get; set; } = default!;
        public string CompanyAcronym { get; set; } = default!;
        public string DeviceAcronym { get; set; } = default!;
        public string DeviceName { get; set; } = default!;
        public int DeviceID { get; set; }
        public int SourcePhaseCount { get; set; }
        public string PhasorLabel { get; set; } = default!;
    ...
    /// </summary>
    public override string? CustomAdapterSettings => CurrentAdapterIndex > -1 && CurrentAdapterIndex < m_adapterDetails.Count ? 
        ...
        using (AdoDataConnection connection = GetConfiguredConnection())
        {
            ...
            foreach (KeyValuePair<int, List<PhaseDetail>> kvp in devicePhaseDetails)
            {
                ...
                List<int> matchingLabelIndexes(string aPhaseLabel, IReadOnlyList<int> phaseIndexes, IReadOnlyList<string> phaseLabels)
                {
                    ...
                    // Check for exact match for best possible case
    ...
    static BulkSequenceCalculator()
    {
        try
        {
            ...
            s_companyAcronym = section["CompanyAcronym", "GPA", "The acronym representing the company who owns the host system."];
