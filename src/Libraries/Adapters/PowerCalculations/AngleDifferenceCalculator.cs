...
public class AngleDifferenceCalculator : CalculatedMeasurementBase
{
    ...
    // Fields
    private readonly double[] m_lastAngles = new double[2];
    private readonly double[] m_unwrapOffsets = new double[2];
    ...
    public override string Status
    {
        get
        {
            ...
            {
                status.Append(m_latestCalculatedAngles.Count > ValuesToShow ? m_latestCalculatedAngles
                    .GetRange(m_latestCalculatedAngles.Count - ValuesToShow, ValuesToShow).Select(v => v.ToString("0.00°")).ToDelimitedString(", ") : 
    ...
    public override void Initialize()
    {
        ...
        if (InputMeasurementKeys is null || InputMeasurementKeyTypes is null)
            throw new InvalidOperationException("No input measurements were specified for the angle difference calculator.");
    ...
    protected override void PublishFrame(IFrame frame, int index)
    {
        ...
        if (TryGetMinimumNeededMeasurements(frame, out IMeasurement[] measurements))
        {
            ...
            for (int i = 0; i < measurements.Length; i++)
            {
                ...
                double angle = measurements[i].AdjustedValue;
            ...
            for (int i = 0; i < measurements.Length; i++)
                m_lastAngles[i] = measurements[i].AdjustedValue;
        }
        else
        {
