...
[UIResource("AdaptersUI", $".DynamicCalculator.DynamicCalculator.main.js")]
[UIResource("AdaptersUI", $".DynamicCalculator.DynamicCalculator.chunk.js")]
public class DynamicCalculator : ActionAdapterBase
{
    ...
    {
        get => m_expressionText ?? string.Empty;
        set
        {
    ...
    {
        get => m_variableList ?? string.Empty;
        set
        {
        ...
        set
        {
            ...
            foreach (string typeDef in value.Split(';'))
            {
                try
                {
                    ...
                    m_expressionContext.Imports.RegisterType(type);
                }
                catch (Exception ex)
                {
    ...
    /// when mixing frame rates and <see cref="TemporalOutlierOperation.PublishWithBadState"/>
    /// when using event-based data such as alarms. If you are not mixing frame rates, consider
    ...
    /// </summary>
    protected ReadOnlyDictionary<string, int> ArrayVariableLengths => new(m_arrayCache.ToDictionary(kvp => kvp.Key, kvp => kvp.Value.Length));

    /// <summary>
    /// Gets variable names mapped to their <see cref="MeasurementKey"/> values.
    /// </summary>
    protected ReadOnlyDictionary<string, MeasurementKey> VariableKeys => new(m_keyMapping.ToDictionary(kvp => GetVariableName(kvp.Value), kvp => kvp.Key));
        
    private static string GetVariableName(in Variable variable) => variable.Index > -1 ? $"{variable.Name}[{variable.Index}]" : variable.Name;
    ...
    private new Ticks RealTime => TimestampSource switch
    {
    ...
    public override void Initialize()
    {
        ...
        if (ExpectsOutputMeasurement && OutputMeasurements?.Length != 1)
            throw new ArgumentException($"Exactly one output measurement must be defined. Amount defined: {OutputMeasurements?.Length ?? 0:N0}");
        ...
        // Assign default value to user defined sentinel value
        m_expressionContext.DefaultValue = SentinelValue;
    ...
    /// </remarks>
    protected virtual void HandleSpecialVariables(IDictionary<string, object> variables)
    {
    ...
    protected void Calculate(IReadOnlyDictionary<MeasurementKey, IMeasurement> measurements, IReadOnlyDictionary<string, int>? indexRestrictions = null)
    {
        ...
        foreach ((MeasurementKey key, Variable variable) in m_keyMapping)
        {
        ...
        foreach ((string name, double[] array) in m_arrayCache)
        {
    ...
    private void AddAliasedVariable(string token)
    {
        ...
        string alias = token[..equalsIndex].Trim();
        string target = token[(equalsIndex + 1)..].Trim();
        ...
        if (alias.EndsWith("[]"))
        {
            ...
            // Split on any comma that is not in parentheses
    ...
    // Gets a measurement key based on a token which may be either a signal ID, measurement key or point tag.
    private MeasurementKey GetKey(string token)
    {
        ...
        if (DataSource is not null && DataSource.Tables.Contains(MeasurementTable))
        {
            ...
            if (rows.Length > 0)
                key = MeasurementKey.LookUpOrCreate(rows[0]["SignalID"].ToNonNullString(Guid.Empty.ToString()).ConvertToType<Guid>(), rows[0]["ID"].ToString() ?? string.Empty);
        }
    ...
    {
        if (value is null)
            throw new InvalidOperationException("Calculation must return a type that can convert to double.");
    ...
    // Helper method to raise the NewMeasurements event when only a single measurement is to be provided.
