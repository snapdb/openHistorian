...
public class DynamicFilter : DynamicCalculator, IFilterAdapter
{
    ...
    private ReadOnlyDictionary<string, MeasurementKey> m_variableKeys = default!;
    ...
    {
        get => base.VariableList ?? string.Empty;
    ...
    public override MeasurementKey[]? InputMeasurementKeys // Overridden to auto-capture measurement key hash set
    {
        ...
        set
        {
            ...
            if (value is null)
                m_inputMeasurementKeys = [];
            else if (!m_inputMeasurementKeys.SetEquals(value))
    ...
    {
        // Not calling base method, filter adapter does not need to concentrate data
        Enabled = true;
    }
    ...
    private List<int> ProcessRemoveWhenTrueForArray(IReadOnlyDictionary<MeasurementKey, IMeasurement> inputs, IReadOnlyDictionary<MeasurementKey, int>? indexes)
    {
        ...
        for (m_index = 0; m_index < m_valueArrayLength; m_index++)
        {
            ...
            if (m_result?.ToString().ParseBoolean() ?? false)
    ...
    /// </remarks>
    protected override void HandleSpecialVariables(IDictionary<string, object> variables)
    {
    ...
    {
        return $"{m_processedMeasurements:N0} measurements processed so far...".CenterText(maxLength);
    }
