...
public class CustomActions
{
    ...
    public static ActionResult AuthenticateServiceAccountAction(Session session)
    {
        ...
        bool isSystemAccount = serviceAccount.Equals("LocalSystem", StringComparison.OrdinalIgnoreCase) || serviceAccount.StartsWith($@"{UserInfo.NTAuthorityGroupName}\", StringComparison.OrdinalIgnoreCase) || serviceAccount.StartsWith($@"{UserInfo.NTServiceGroupName}\", StringComparison.OrdinalIgnoreCase);
        ...
        else if (isManagedServiceAccount)
        {
            ...
            bool isManagedServiceAccountValid = serviceAccountInfo.Exists && !serviceAccountInfo.AccountIsDisabled && !serviceAccountInfo.AccountIsLockedOut && serviceAccountInfo.GetUserPropertyValue("msDS-HostServiceAccountBL").Split(',')[0].Equals("CN=" + Environment.MachineName, StringComparison.CurrentCultureIgnoreCase);
        ...
        if (File.Exists(filePath))
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, string> mapping in mappings.ParseKeyValuePairs())
                {
                    ...
                    if (mode != "WRITE")
                    {
                        session[mapping.Key] = query.OfType<XElement>().Select(element => element.Value).Concat(query.OfType<XAttribute>().Select(attribute => attribute.Value)).FirstOrDefault();
                    }
                    else
                    {
    ...
    public static ActionResult CompanyInfoAction(Session session)
    {
        ...
        if (File.Exists(configPath))
        {
            ...
            foreach (XElement systemSettings in config.Descendants("systemSettings"))
            {
                ...
                {
                    systemSettings.Add(new XElement("add", new XAttribute("name", "CompanyName"), new XAttribute("value", companyName), new XAttribute("description", $"The name of the company who owns this instance of the {serviceName}."), new XAttribute("encrypted", "false")));
                }
                ...
                {
                    systemSettings.Add(new XElement("add", new XAttribute("name", "CompanyAcronym"), new XAttribute("value", companyAcronym), new XAttribute("description", $"The acronym representing the company who owns this instance of the {serviceName}."), new XAttribute("encrypted", "false")));
                }
                ...
                {
                    systemSettings.Add(new XElement("add", new XAttribute("name", "SystemName"), new XAttribute("value", systemName), new XAttribute("description", $"Name of system for this instance of the {serviceName} that will be prefixed to system level tags, when defined. Value should follow tag naming conventions, e.g., no spaces and all upper case."), new XAttribute("encrypted", "false")));
                }
            }
    ...
    public static ActionResult DatabaseQueryAction(Session session)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.Log(InstallMessage.Error, $"Library Path: {dllPath}{Environment.NewLine}" + $"Database Query: {query}{Environment.NewLine}" + $"Connection string: {connectionString}{Environment.NewLine}" + $"Data provider string: {dataProviderString}");
    ...
    public static ActionResult DatabaseScriptAction(Session session)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.Log(InstallMessage.Error, $"Database Script: {scriptPath}{Environment.NewLine}" + $"Connection string: {connectionString}{Environment.NewLine}" + $"Data provider string: {dataProviderString}");
    ...
    public static ActionResult StartProcessAction(Session session)
    {
        ...
        try
        {
            ...
            findAndExecute("WaitForExit", _ =>
            {
                ...
                process.OutputDataReceived += (_, args) => logger.Log(args.Data);
    ...
    private static void SetFolderReadOnly(string folderPath)
    {
        try
        {
            ...
            FileSystemAccessRule readOnlyRule = new FileSystemAccessRule(currentUser, FileSystemRights.ReadAndExecute | FileSystemRights.ListDirectory, InheritanceFlags.ContainerInherit | InheritanceFlags.ObjectInherit, PropagationFlags.None, AccessControlType.Allow);
    ...
    {
        return !session.GetMode(InstallRunMode.Scheduled) && !session.GetMode(InstallRunMode.Rollback) && !session.GetMode(InstallRunMode.Commit);
    }
    ...
    private class Logger
    {
        ...
        public Logger(Session session) : this(session, GetPropertyValue(session, "SERVICENAME"))
        {
        }
    ...
    {
        // Reference resource assembly - this call simply ensures a code reference exists
        // to the 'Gemstone.InstallerActions.Resources' assembly so that it is initialized
        // before any custom actions are executed. The resource assembly loads needed GSF
        // assemblies into the application domain using a module initializer.
