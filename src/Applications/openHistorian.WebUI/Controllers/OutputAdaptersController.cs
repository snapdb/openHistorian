...
{
    /// <summary>
    /// Gets all Assemblies holding output adapters.
    /// </summary>
    ...
    public async Task<IActionResult> GetAssemblies(CancellationToken cancellationToken)
    {
        ...
        return await Task.Run(() =>
            Ok(AdapterCollectionHelper<IOutputAdapter>.GetAdapters().Select(adapter => new ValueLabel()
            {
    ...
    /// <summary>
    /// Gets all output Adapters from a specified Assembly.
    /// </summary>
    ...
    public async Task<IActionResult> GetTypes(string assemblyName, CancellationToken cancellationToken)
    {
        ...
        return await Task.Run(() =>
            Ok(AdapterCollectionHelper<IOutputAdapter>.GetAdapters().Where((a) => String.Compare(a.Assembly, WebUtility.UrlDecode(assemblyName), true) == 0)
    ...
    public async Task<IActionResult> GetConnectionStringParameters(string encodedAssemblyName, string encodedTypeName, CancellationToken cancellationToken, int? id = null)
    {
        ...
        if (id is not null)
        {
            ...
            TableOperations<CustomOutputAdapter> tableOperations = new(connection);
            CustomOutputAdapter? result = await tableOperations.QueryRecordAsync(new RecordRestriction($"{PrimaryKeyField} = {{0}}", id), cancellationToken);
        ...
        IEnumerable<ConnectionParameter> connectionParameters = AdapterCollectionHelper<IOutputAdapter>.GetConnectionParameters(assemblyName, typeName, connectionString);

        return Ok(connectionParameters);
    }

    /// <summary>
    /// Sets the ConnectionString of a <see cref="CustomOutputAdapter"/> based on a <see cref="IEnumerable{ConnectionParameter}"/>
    /// </summary>
    ...
    /// <summary>
    /// Retrieves the JavaScript module containing UI resources for the specified output adapter.
    /// </summary>
    ...
    public async Task<IActionResult> ProxyToAdapter(string encodedAssemblyName, string encodedTypeName, string encodedResourceName, CancellationToken cancellationToken)
    {
        ...
        return await Task.Run(() =>
        {
            Dictionary<string, Func<ControllerBase, IActionResult>> adapterDictionary = AdapterCollectionHelper<IOutputAdapter>.GetUIResources(typeName, assemblyName);
